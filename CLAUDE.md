...
**Important duplication:** `InfrastructurePipelineStack` defines an internal `InfrastructureStage` that re-instantiates `CertificateStack`, `WebsiteStack`, and `EmailStack`. So those three stacks exist twice in the CDK app — once top-level (deployed by `cdk deploy`) and once inside the pipeline stage (deployed by the pipeline). Changing a stack's props means changing them in **both** `bin/infrastructure.ts` and `lib/infrastructure-pipeline-stack.ts`, or the two deployment paths drift.

The stage stacks each set an explicit `stackName` (`CertificateStack`, `PersonalWebsiteStack`, `EmailStack`, `FrontendPipelineStack`) matching the top-level ones, so both paths target the *same* CloudFormation stacks and the pipeline updates them rather than creating a second copy. **Do not remove those `stackName` overrides.**

Because the stage adopts the live stack names, **it must mirror the entire app** — including `FrontendPipelineStack`, which is why that stack is instantiated in the stage despite not being "infrastructure". Cross-stack references only produce CloudFormation Exports when a consumer exists in the same synth. Leave `FrontendPipelineStack` out and `PersonalWebsiteStack` synthesizes with zero Outputs, so deploying it tries to delete the three exports the real `FrontendPipelineStack` imports (`ExportsOutputRefSiteDistribution…`, `ExportsOutputRefSiteBucket…`, `ExportsOutputFnGetAttSiteBucket…Arn`) and CloudFormation rejects the update. When adding a stack to `bin/infrastructure.ts` that produces or consumes a cross-stack reference, add it to the stage too — and when verifying, diff both `Resources` *and* `Outputs` between the two synthesized templates. Without them a `cdk.Stage` prefixes the stage id, yielding `InfrastructureStage-EmailStack` etc., which cannot coexist with the originals — `EmailStack` hardcodes the bucket name `matteo.wtf-email-forwarding` and the SES rule set name, and `WebsiteStack` hardcodes the bucket `www.matteo.wtf` and its Route 53 A record, all globally unique. That collision is what left the old duplicate `EmailStack` stuck in `ROLLBACK_COMPLETE` and permanently failing the stage.

**Push triggering:** `matteo-frontend-pipeline` is a V2 pipeline, where push detection comes from the pipeline-level `triggers` block. The source action's `DetectChanges` field is a V1 mechanism and is ignored there — without `triggers` the pipeline never fires on a push. `matteo-infrastructure-pipeline` is V1 and relies on `DetectChanges` (default true). Change detection also requires the **AWS Connector for GitHub** App to be installed on the repo and delivering webhooks; if pushes stop starting runs while `Source` still succeeds when started manually, the App's webhook delivery is the thing to check, not the pipeline config — source *download* and *change detection* are separate permissions.

Both pipelines source from GitHub `hpfs74/personal-website` `main` via a **CodeConnections (GitHub App) connection**, defined once in `lib/source-config.ts`. The connection does not expire. It replaced a classic PAT in the Secrets Manager secret `gh-token`: PATs expire, and when that one lapsed both pipelines failed at Source with "Could not access the GitHub repository". `gh-token` is no longer referenced by this repo. If the source ever fails auth again, check the connection is `AVAILABLE` and that the GitHub App installation still grants access to the repo — don't reintroduce a token.

- Frontend pipeline builds with `buildspec-frontend.yml`: `npm ci` → `npm run test:run` → `npm run build` → deploy `dist/` to S3 → CloudFront invalidation. **Tests gate deploys.** Both CodeBuild projects are `PipelineProject`s — their source is always the artifact CodePipeline hands them, so they need no GitHub credentials of their own.
